...
use crate::errors::AppError; // Assuming AppError is accessible
...
mod tests {
    use super::*;
    use crate::model::donation::donation::{Donation, NewDonationRequest}; // Ensure this path is correct
    ...
    impl DonationRepository for MockDonationRepository {
        async fn create(&self, user_id: i32, new_donation: &NewDonationRequest) -> Result<Donation, AppError> {
            ...
            let donation = Donation {
                ...
                created_at: Utc::now(),
                // updated_at: Utc::now(), // Assuming Donation model has updated_at, if not, remove
            };
        ...
        async fn update_message(&self, donation_id: i32, user_id: i32, message: Option<String>) -> Result<u64, AppError> {
            ...
            if let Some(donation) = state.donations.get_mut(&donation_id) {
                if donation.user_id == user_id {
                    ...
                    // donation.updated_at = Utc::now(); // If Donation struct has updated_at
                    Ok(1)
                } else {
                    Ok(0)
                }
        ...
        async fn get_user_total_for_campaign(&self, user_id: i32, campaign_id: i32) -> Result<i64, AppError> {
            ...
            let total_amount = state
                .donations
                .values()
                .filter(|d| d.user_id == user_id && d.campaign_id == campaign_id)
                .map(|d| d.amount)
                .sum();
            Ok(total_amount)
        }
    ...
    async fn test_find_by_campaign() {
        ...
        repo.create(2, &NewDonationRequest { campaign_id: campaign_id_target, amount: 20, message: Some("Msg1".into()) }).await.unwrap();
        ...
        assert!(donations.iter().any(|d| d.amount == 10));
        assert!(donations.iter().any(|d| d.amount == 20));

        let donations_other = repo.find_by_campaign(campaign_id_other).await.unwrap();
        assert_eq!(donations_other.len(), 1);
        assert_eq!(donations_other[0].amount, 30);
    ...
    async fn test_find_by_user() {
        ...
        repo.create(user_id_target, &NewDonationRequest { campaign_id: 302, amount: 25, message: Some("Msg2".into()) }).await.unwrap();
    ...
    async fn test_get_total_donated_for_campaign_multiple_donations() {
        ...
        repo.create(2, &NewDonationRequest { campaign_id, amount: 50, message: None }).await.unwrap();
    ...
    async fn test_get_total_donated_for_campaign_no_donations() {
        let repo = get_repository();
        let campaign_id = 503; // Campaign with no donations
    ...
    async fn test_get_total_donated_for_campaign_single_donation() {
        ...
        repo.create(1, &NewDonationRequest { campaign_id, amount: 75, message: None }).await.unwrap();
    ...
    async fn test_get_user_total_for_campaign_multiple_donations_by_user() {
        ...
        repo.create(user_id, &NewDonationRequest { campaign_id: 602, amount: 100, message: None }).await.unwrap(); // Same user, different campaign
        repo.create(72, &NewDonationRequest { campaign_id, amount: 50, message: None }).await.unwrap(); // Different user, same campaign
    ...
    async fn test_get_user_total_for_campaign_no_donations_by_user_for_campaign() {
        ...
        // User 73 donates to another campaign
        repo.create(user_id, &NewDonationRequest { campaign_id: 604, amount: 30, message: None }).await.unwrap();
        // Another user donates to campaign 603
        repo.create(74, &NewDonationRequest { campaign_id, amount: 60, message: None }).await.unwrap();
    ...
    async fn test_get_user_total_for_campaign_single_donation_by_user() {
        ...
        repo.create(user_id, &NewDonationRequest { campaign_id, amount: 90, message: None }).await.unwrap();
